...
export default function ReferencesPage() {
  return (
    <main className="min-h-screen overflow-x-hidden bg-[var(--bg)] px-3 py-6 text-[var(--text)] sm:px-5 sm:py-10">
      <div className="mx-auto w-full max-w-[760px]">
        <h1 className="break-words text-[19px] font-extrabold leading-tight sm:text-[22px]">
          NSCP 2015 / ACI 318 Design Reference
        </h1>
        <p className="mt-2 break-words text-[11px] leading-relaxed text-[var(--text-muted)] sm:text-[12px]">
          Organized to mirror the {modules.length} learning modules. Each chapter below
          corresponds to a module in <code className="break-all text-[10px]">lib/data.ts</code>.
        </p>

        {/* Table of contents, pulled live from modules[] */}
        <nav className="mt-5 min-w-0 rounded-lg border border-[var(--border)] bg-[var(--bg-surface)] p-3 sm:p-4">
          ...
          <ol className="mt-2 space-y-1.5">
            {modules.map((m) => (
              <li key={m.slug} className="min-w-0">
                <a
                  href={`#ch-${m.slug}`}
                  className="block break-words text-[11px] leading-relaxed text-[var(--blue)] hover:underline sm:text-[12px]"
                >
        ...
        <div className="mt-6 space-y-8 sm:space-y-10">
          {chapters.map((chapter) => {
            ...
            return (
              ...
              >
                <h2 className="break-words border-b border-[var(--border)] pb-2 text-[14px] font-extrabold leading-relaxed sm:text-[16px]">
                  Module {mod.index}. {mod.title}
                </h2>
                <p className="mt-1.5 break-words text-[10px] leading-relaxed text-[var(--text-muted)] sm:text-[11.5px]">
                  {mod.description}
                </p>
                ...
                <div className="mt-4 space-y-3">
                  {chapter.sections.map((section) => (
                    <article
                      key={section.title}
                      className="min-w-0 overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--bg-surface)] p-3 sm:p-4"
                    >
                      <div className="flex min-w-0 flex-col items-start gap-1.5 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                        <h3 className="min-w-0 break-words text-[13px] font-bold leading-relaxed text-[var(--text)] sm:flex-1 sm:text-[13.5px]">
                          {section.title}
                        </h3>
                        <span className="block min-w-0 max-w-full break-words text-[9px] leading-relaxed text-[var(--blue)] sm:max-w-[48%] sm:flex-shrink-0 sm:text-right sm:text-[10px]">
                          {section.reference}
                        </span>
                      </div>
                      <p className="mt-3 break-words text-[11px] leading-relaxed text-[var(--text-muted)] sm:mt-2 sm:text-[12px]">
                        {section.summary}
                      </p>
                      {section.equations && (
                        <div className="min-w-0 max-w-full overflow-x-auto">
                          <EquationTable eq={section.equations} />
                        </div>
                      )}
                      {section.table && (
                        <div className="min-w-0 max-w-full overflow-x-auto">
                          <CodeTable table={section.table} />
                        </div>
                      )}
                    </article>
                  ))}
        ...
        <p className="mt-8 break-words text-[9px] leading-relaxed text-[var(--text-muted)] sm:text-[10px]">
