export interface RectangularBeamAnalysisInput {
  b: number;
  d: number;
  dPrime: number;
  fc: number;
...
export interface RectangularBeamAnalysisTensionLayerResult {
  index: number;
  area: number;
  depth: number;
  strain: number;
...
export interface RectangularBeamAnalysisResult {
...
function phiFromStrain(
  ...
  phi: number;
  ductilityClass: RectangularBeamAnalysisResult["ductilityClass"];
} {
  if (epsilonT >= 0.005) {
...
export function getRectangularBeamAnalysisSolutionSteps(
  input: RectangularBeamAnalysisInput,
