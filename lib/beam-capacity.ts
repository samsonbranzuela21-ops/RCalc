export interface BeamCapacityInput {
  ...
  dPrime: number;   // effective depth to compression steel, mm (only used if AsPrime > 0)
  fc: number;       // f'c, MPa
  fy: number;       // fy, MPa
  As: number;       // provided tension steel area, mm²
  AsPrime: number;  // provided compression steel area, mm² (0 = singly reinforced)
  Mu: number;       // applied factored moment to check against, kN·m
}
...
export interface BeamCapacityResult {
  ...
  fsPrime: number | null;    // MPa, stress in compression steel (null if singly reinforced)
  compressionSteelYields: boolean | null;
  epsilonT: number;          // strain in tension steel
  epsilonY: number;          // yield strain, fy/Es
  phi: number;
  ductilityClass: "tension-controlled" | "transition" | "compression-controlled";
  rho: number;
  rhoMin: number;
  rhoMax: number;
  Mn: number;                // kN·m, nominal moment capacity
  phiMn: number;             // kN·m, design moment capacity
  Mu: number;                // kN·m, applied factored moment (pass-through)
  utilizationRatio: number;  // Mu / phiMn
  ok: boolean;               // phiMn >= Mu
  message: string;
}
...
function phiFromStrain(epsilonT: number, epsilonY: number): { phi: number; ductilityClass: BeamCapacityResult["ductilityClass"] } {
  if (epsilonT >= 0.005) {
  ...
  const phi = 0.65 + (epsilonT - epsilonY) * (0.25 / (0.005 - epsilonY));
  return { phi, ductilityClass: "transition" };
}

export function checkBeamCapacity(input: BeamCapacityInput): BeamCapacityResult {
  const { b, d, dPrime, fc, fy, As, AsPrime, Mu } = input;
  ...
  if (!isDoublyReinforced) {
    ...
    // Doubly reinforced — assume compression steel yields first
    const aAssumed = (As * fy - AsPrime * (fy - 0.85 * fc)) / (0.85 * fc * b);
    const cAssumed = aAssumed / beta1;
    const epsilonSPrimeAssumed = (0.003 * (cAssumed - dPrime)) / cAssumed;

    if (epsilonSPrimeAssumed >= epsilonY) {
      // Compression steel yields — assumption confirmed
      a = aAssumed;
      c = cAssumed;
      fsPrime = fy;
      compressionSteelYields = true;
    } else {
      // Compression steel does NOT yield — solve quadratic in c
      const A = 0.85 * fc * b * beta1;
      const B = 0.003 * ES * AsPrime - 0.85 * fc * AsPrime - As * fy;
  ...
  const { phi, ductilityClass } = phiFromStrain(epsilonT, epsilonY);
  ...
  if (!isDoublyReinforced) {
    ...
    MnNmm = Cc * (d - a / 2) + CsPrime * (d - dPrime);
  }

  const Mn = MnNmm / 1e6; // kN·m
  const phiMn = phi * Mn;
  const utilizationRatio = Mu > 0 ? Mu / phiMn : 0;
  const ok = phiMn >= Mu;

  let message: string;
  if (ductilityClass === "compression-controlled") {
    message = "Section is compression-controlled (brittle) — not recommended; increase steel ductility or reduce As.";
  } else if (!ok) {
    message = `Section is INADEQUATE — φMn (${phiMn.toFixed(1)} kN·m) < Mu (${Mu.toFixed(1)} kN·m).`;
  } else {
    message = `Section is adequate — φMn (${phiMn.toFixed(1)} kN·m) ≥ Mu (${Mu.toFixed(1)} kN·m).`;
  }

  return {
    isDoublyReinforced, beta1, a, c, fsPrime, compressionSteelYields,
    epsilonT, epsilonY, phi, ductilityClass,
    rho, rhoMin, rhoMax,
    Mn, phiMn, Mu, utilizationRatio, ok, message,
  };
}
...
export function getBeamCapacitySolutionSteps(
  ...
  const steps: BeamCapacitySolutionStep[] = [
    {
      ...
      substitution: fc <= 28 ? "" : `\\beta_1 = 0.85 - 0.05\\left[\\dfrac{${fc}-28}{7}\\right]`,
    ...
    steps.push(
      {
        ...
        formula: "a = \\dfrac{A_s f_y - A_s'(f_y - 0.85f'_c)}{0.85 f'_c b}",
      ...
      },
      {
        ...
        formula: "\\varepsilon_s' = \\dfrac{0.003(c-d')}{c} \\ ; \\ \\text{yields if } \\varepsilon_s' \\ge \\varepsilon_y",
  ...
  steps.push(
    ...
    },
    {
      ...
      substitution: `\\phi M_n = ${r.phiMn.toFixed(2)} \\text{ kN·m vs. } M_u = ${Mu} \\text{ kN·m}`,
      result: r.ok ? "\\text{OK — section adequate}" : "\\text{NOT OK — section inadequate}",
    }
  );
